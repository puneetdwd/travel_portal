...
<script type="text/javascript">
    ...
    $('#top_count,#city_id').change(function () {
        get_topper();
    });
    ...
    function get_topper() {
        ...
        $.ajax({
            url: "<?php echo base_url('report/get_hotel_toppers'); ?>",
            type: "POST",
            dataType: "html",
            data: {
    ...
    function pie() {
        var data = google.visualization.arrayToDataTable([
            ['Hotel Name', 'Total Trip'],
<?php
foreach ($hotel as $key => $value) {
    if ($value['name'] != '' && $value['total_stay'] != '') {
        ?>
                    ['<?php echo $value['name']; ?>',<?php echo $value['total_stay']; ?>],
        <?php
    }
}
?>
