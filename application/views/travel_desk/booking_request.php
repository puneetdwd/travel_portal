...
<div class="page-content"><div class="row hidden-xs">
...
<!-- ngIf: $index!=arrStates.length-1 -->
<div style="display:inline-block;height:70px;vertical-align:top" ng-if="$index != arrStates.length - 1" class="ng-scope" data-original-title="" title="">
<div style="border:1px solid black;width:70px;margin-top:35px;" data-original-title="" title=""></div></div>
<!-- end ngIf: $index!=arrStates.length-1 -->
</div>
<!-- end ngRepeat: state in arrStates track by $index -->
<div ng-repeat="state in arrStates track by $index" style="display:inline-block;vertical-align:top" class="ng-scope" data-original-title="" title="">
...
<div ng-repeat="state in arrStates track by $index" style="display:inline-block;vertical-align:top" class="ng-scope" data-original-title="" title="">
<div style="padding:10px;text-align:center;display:inline-block;width:100px;" data-original-title="" title="">
<div style="width: 50px; height: 50px; color: white; padding: 17px; margin: 0px auto; border-radius: 50px !important; background-color: #e73d4a;" ng-style="$parent.getBackgroundColor($index)" class="ng-binding" data-original-title="" title=""></div>
<span class="ng-binding"> Approver Reject</span>
</div>
<!-- ngIf: $index!=arrStates.length-1 -->
</div>
<?php } ?>
...
<spam class="cutm_lbl btn_blue">Train Ticket</spam>
<span style="font-size: 13px;">(<?php echo $request['from_city_name'] ?> To <?php echo $request['to_city_name'] ?>)</span>
...
<h4 class="form-section marginLeft15">
<spam class="cutm_lbl btn_blue">Bus Ticket</spam>
<span style="font-size: 13px;">(<?php echo $request['from_city_name'] ?> To <?php echo $request['to_city_name'] ?>)</span>
<a href="#cancelForm" onClick="setSubjectId('bus');" data-toggle="modal"><span class="cutm_lbl btn_red pull-right">Reject</span></a>
</h4>
...
<input name="check_in_date" id="check_in_date" class="form-control" type="text" readonly>
<span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span></div>
...
<div class="row"><div class="col-md-12"><div class="form-group col-xs-12">
...
<option value='3'>Auto</option></select></div>
<div class="form-group col-xs-12">
...
<option value='2'>Full Day</option></select></div>
...
<span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span></div>
<label id="departure_date-error" class="error" for="departure_date"></label></div>
...
<span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span></div>
<label id="drop_off_date-error" class="error" for="drop_off_date"></label></div>

<div class="form-group col-xs-12">
<label class="control-label text-left-imp">Pick-Up Location<span class="required"> * </span>:</label>                                    
<input type="text" class="form-control required" id="pick_up_location" name="pick_up_location" value=""></div>

<div class="form-group col-xs-12">
<label class="control-label text-left-imp">Drop-Off Location<span class="required"> * </span>:</label>                                    
<input type="text" class="form-control required" id="drop_off_location" name="drop_off_location"  value=""></div>

<div class="form-group col-xs-12">
<label class="control-label text-left-imp">Cost@Per KM:</label>                                                                            
<input type="text" class="form-control" id="car_rent" disabled=""></div>

<div class="form-group col-xs-12">
<label class="control-label text-left-imp">Total Cost<span class="required"> * </span>:</label>                                                                            
<input type="number" class="form-control" name="cost" id="cost"></div>

<div class="form-group col-xs-12">
<label class="control-label text-left-imp">Attach:</label>                                                                            
<input type="file" class="form-control" name="car_attchment"></div>

<div class="form-actions col-xs-12">
<label class="control-label text-left-imp">&nbsp;</label>
<input type="hidden" name="request_id" value="<?php echo $request['request_id']; ?>">
<input type="hidden" name="booking_city_id" value="<?php echo $booking_city_id; ?>">
<button class="btn green form-control" type="submit">Submit</button></div></div>
</div><div class="row hidden-xs"><br><br><br><br><br><br><br></div></form></div><?php
}
}
...
<h4 class="form-section marginLeft15">
<spam class="cutm_lbl btn_blue">Return Flight Ticket</spam>
...
<h4 class="form-section marginLeft15">
<spam class="cutm_lbl btn_blue">Return Train Ticket</spam>
<span style="font-size: 13px;">(<?php echo $request['to_city_name'] ?> To <?php echo $request['from_city_name'] ?>)</span>
...
<div class="col-md-12">
<div class="form-group col-xs-12">
<label class="control-label text-left-imp">Food<span class="required">*</span>:</label>                                    
<input placeholder="RS." type="number" min="0" class="form-control required" name="foods" id="foods">
</div>
<div class="form-group col-xs-12">
<label class="control-label text-left-imp">Local conveyance<span class="required">*</span>:</label>                                    
<input placeholder="RS." type="number" min="0" class="form-control required" name="travel" id="travel">
</div>
<!--<div class="form-group col-xs-12">
...
<label title="Employee would be entitled for DA@50% of the entitlemant" class="control-label text-left-imp">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;arranged by Dainik Bhaskar (DA is NOT admissible)</label></d
...
$("#reaching_date").change(function () {

var startDate1 = $('#boarding_date').val().split(' ');
var sD= startDate1[0].split('-');
var startDate = sD[1]+'-'+sD[0]+'-'+sD[2]+' '+startDate1[1]+':00';
var b= new Date(startDate);
if($('#reaching_date').val()!='')
 {
  var endDate1 = $('#reaching_date').val().split(' ');
  var eD= endDate1[0].split('-');
  var endDate = eD[1]+'-'+eD[0]+'-'+eD[2]+' '+endDate1[1]+':00';
  var a= new Date(endDate);
  if(Date.parse(a)==Date.parse(b))
   {
	alert("Boarding date/time and reaching date/time should not be same.");
	$('#reaching_date').val('');
   }
  else
   {
    if((Date.parse(a) < Date.parse(b)))
     {
	  alert("Reaching date/time should be greater than Boarding date/time");
	  $('#return_date').val('');
     }
   }
 }
