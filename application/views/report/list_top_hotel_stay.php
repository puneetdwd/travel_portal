...
<thead><tr><th>Total<br> Trip</th>
<th>Hotel/GH<br> Name</th>
<th>Location</th>
<th>Single<br> Occupancy Rate</th>
<th>Double<br> Occupancy Rate</th>
<th>Hotel/GH<br> Category</th>
<th>Type</th></tr></thead><tbody><?php
foreach ($hotel as $data) {
?><tr><td><?php echo $data['total_stay']; ?></td>
<td><a title="Show Visitors" href="javascript:void(0)" data-toggle="modal" data-target="#visitor_modal" onClick="ShowVisitors('<?php echo $data['id']; ?>');"><?php echo $data['name']; ?></a></td>
<td><?php echo $data['city_name']; ?></td>
<td><?php echo $data['half_amount']; ?></td>
<td><?php echo $data['amount']; ?></td><td><?php
if ($data['category'] == "1") {
echo "1 Star";
...
function ShowVisitors(hotelID)
{
 $("#visitorDataHolder").html("<div class='alert alert-info'>Loading...</div>");
	$.ajax({
		url: "<?php echo base_url('report/get_visitors'); ?>",
		type: "POST",
		dataType: "html",
			data: {
				city_id: $("#city_id").val(),
				...
				dept_id: $("#dept_id").val(),
				type: $("#type").val(),
				hotel_or_GH_id: hotelID
			},
			catch : false,
			success: function (data) {
				$("#visitorDataHolder").html(data);
				pie();
            },
			error: function () {
				$("#visitorDataHolder").html("<div class='alert alert-danger'><i class='fa fa-times'></i>Something went wrong, please try again later.</div>");
			}
		});
}
