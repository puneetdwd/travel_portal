...
function find_da_multiple($hrs)
 {
  $DA_multiple = 0;
  if($hrs>=6 and $hrs<=13){$DA_multiple=0.5;}
  elseif($hrs>=14 and $hrs<=24){$DA_multiple=1;}
  return $DA_multiple;
 }
...
function find_all_dates_between_2_dates($fromd, $tod)
 {
  ...
  foreach($daterange as $date){$allDates[] = $date->format("d-m-Y");}
  $allDates[] = $tod;
  return $allDates;
 }

function find_DA_rows_OLD_BACKUP($fromDate, $toDate, $cityGrade, $empGrade)
 {
...
function find_DA_rows($fromDate, $toDate, $cityGrade, $empGrade)
 {
  $allDatesWithMutiplicationFactor = array();
  $Begin = explode(' ', $fromDate);
  $Finish = explode(' ', $toDate);
  ...
  foreach($allDuringDates as $k=>$v)
   {
	...
	if($lastTimeStamp>$next24Hr){$iterationNext = date('d-m-Y H:i', $next24Hr);}
	else{$iterationNext = $toDate;}
...
<div id="print_div"><div class="header text-center">
...
<th width="15%">Employee Id</th><td width="16%"><?php echo $employee['employee_id'] ?></td>
<th width="15%">Designation</th><td width="16%"><?php echo $employee['desg_name'] ?></td></tr>
<tr><th>Grade</th><td><?php echo $employee['grade_name'] ?></td>
<th>Reporting Manager</th><td><?php echo $employee['reporting_manager'] ?></td>
<th>Purpose of Travel</th>
<td><?php
if($request['project_id']!=""){echo "Project";}else{echo $request['reason'];}
if($request['project_id']!=""){echo ', ' . $project['name'];}?></td></tr>

<tr><th>Departure Date and Time</th><td><?php echo date(DATETIME_FORMAT, strtotime($request['departure_date'])); ?></td>
<th>From-To</th><td><?php echo $request['from_city_name'] . ' - ' . $request['to_city_name']; ?></td>
<th>Credit Card Number</th><td><b><?php echo $expense_pending['credit_card_number'];?></b></td></tr>

<tr><th>Return Date and Time</th><td><?php if($request['trip_type']!="1"){ echo date(DATETIME_FORMAT, strtotime($request['return_date'])); } ?></td>
<th>Travel Type</th><td><?php if($request['group_travel']=="1"){echo "Group Travel";}else{echo "Single Person";}?></td>
<th>Bank Name</th><td><b><?php echo $expense_pending['bank_name']; ?></b></td></tr></tbody>
</table></div></div></div>
...
if($eligibility_mode!=''){echo $toSetRedFlag1 = $eligibility_mode . "/" . $eligibility_class;}
else{if($sel_traverl_class!=''){echo $toSetRedFlag1 = $travel_mode . "/" . $sel_traverl_class;}
else{echo $toSetRedFlag1 = $travel_mode;}}?></span>)</h4>
...
<th>Location To</th><th>Expense Location</th><th>Paid By</th><th>Mode</th><th>View</th>
<th>Cost</th><th>Tax</th><th>Agency charges</th><th>Total</th></tr></thead><tbody><?php
$i = 1;
$total = 0;
foreach($ticket_details as $key=>$value)
 {
  ...
  <td><?php if(isset($value['expense_location'])) echo $value['expense_location'] ?></td>
  <td><?php echo $value['arrange_by'] ?></td><td><?php echo $thisRED; ?></td><td><?php
  if($value['attachment']!='')
   {
  ...
  if($value['arrange_by']=='Own')
   {
	$OwnExpenses[] = $value_cost;
   }
  if($value['arrange_by']=='Company')
   {
	$expensesByCompany[] = $value_cost;
   }
  ?></td></tr><?php
 }
foreach($other_trip_expense as $key=>$value)
 {
  ...
  elseif($service_type=="2"){$thisRED="Train";}
  elseif($service_type=="3"){$thisRED="Car";}
  elseif($service_type=="4"){$thisRED="Bus";}
  elseif($service_type=="5"){$thisRED = "Hotel";}
  if(isset($value['travel_class'])){$thisRED=$thisRED.'/'.$value['travel_class'];}
  if($thisRED!=$toSetRedFlag1){$checkRed='danger';}
  ?><tr class="<?php echo $checkRed; ?>"><td><?php echo $i++; ?></td>
  <td><?php echo date(DATETIME_FORMAT, strtotime($value['trip_date'])); ?></td>
  <td><?php echo $value['trip_from']; ?></td><td><?php echo $value['trip_to']; ?></td>
  <td><?php //echo $value['trip_arrange_by'];?></td>
  <td><?php echo $value['trip_arrange_by'] ?></td><td><?php
  if($value['trip_book_by']=="1"){echo "Flight";}
  elseif($value['trip_book_by']=="2"){echo "Train";}
  elseif($value['trip_book_by']=="3"){echo "Car";}
  elseif($value['trip_book_by']=="4"){echo "Bus";}
  elseif($value['trip_book_by']=="5"){echo "Hotel";}
  ?></td><td><?php
  $view = 1;
...
<td><b id="txt_total_sum"><?php echo $total . '.00'; ?></b></td></tr></tfoot></table></div></div></div>

<div class="row"><div class="col-md-12 light bordered">
<h4 class="form-section">Guest House/Hotel/Own Arrangement(<span style="color:#005982;">Eligibility-&#8377;<?php
$GH_act = $hotel_allowance;
if($GH_act==0){echo 'Actual';}
else{echo $GH_act . '/Day';}
?></span>)</h4>
<div class="row">
<table id="loading_hotel" class="table table-hover table-bordered text-center">
<thead><tr class="th_blue"><th>Sr.No.</th><th>Hotel</th><th>Check-In<br> Date</th>
<th>Check-Out<br> Date</th><th>Room No</th><th>Bill No</th><th>Location</th>
<th>Expense<br> Location</th><th>View</th><th>Paid By</th><th>Hotel Expense</th>
...
?><tr class="<?php echo $trClassHotel1; ?>"><td><?php echo $i; ?></td><td><?php echo $value['hotel_provider_name']; ?></td>
<td><?php echo date(DATETIME_FORMAT, strtotime($value['date_from'])); ?></td>
<td><?php echo date(DATETIME_FORMAT, strtotime($value['date_to'])); ?></td>
<td><?php echo $value['bill_no']; ?></td><td><?php echo $value['bill_no_1']; ?></td>
<td><?php echo $value['location'] ?></td><td><?php echo $value['expense_location'] ?></td>
<td><?php
if($value['attachment']!='')
 {
  ?><a class="btn-link" target="_blank" href="<?php echo base_url() . $this->config->item('upload_booking_attch_path').'/'.$value['attachment']; ?>"><i class="fa fa-eye"></i> View </a><?php
...
<b <?php echo $sty; ?> ><?php echo $total1 . '.00'; ?></b></td></tr></tfoot>
...
if($hours!='0'){
if($hours!=''){
if($hours<14){$da=$request['DA_allowance']/2;
$da_eligibility=$da_eligibility+$da;}
else{$da_eligibility=$da_eligibility+$request['DA_allowance'];}
}}
...
<h4 class="form-section">DA Perticulars(<span style="color:#005982;">Eligibility-&#8377;<?php echo $da_eligibilityVIEW; ?></span>)</h4>
<div class="row"><?php
$i = 1;
$total2 = 0;
...
if($DA_50==3){echo ' (Policy : DA is Not Admissible)';
$policy = ' (Not Admissible)';
}
elseif($DA_50==1){
echo ' (Policy : Guest house with</br>food DA@50%)';
$policy = ' (DA@50%)';
}
elseif($DA_50==2){
echo ' (Policy : Hotel with food DA@50%)';
$policy = ' (DA@50%)';
}}
?>">DA/day <?php echo $policy; ?></th>
...
}else{if(!empty($expense_details)){$to_add_sustract = $request['DA_allowance'];}
else{$to_add_sustract = 0;}}
if(isset($DA_50) and $DA_50>0){
if($DA_50==3){$to_add_sustract=0;}
elseif($DA_50==1 or $DA_50==2){$to_add_sustract=$to_add_sustract/2;}}
...
<input disabled class="da_cb" rel="<?php echo $to_add_sustract; ?>" rel2="<?php echo 'rec_' . $i; ?>" value="1" type="checkbox" name="<?php echo 'claim_this_' . $i; ?>" <?php
if($val['donate']==0){echo 'checked="checked"';}
?> /><?php echo $val['serial']; ?><input type="hidden" value="<?php echo $val['serial']; ?>" name="da_claims_serial[]" readonly /></td>
...
<td><?php
if($request['DA_allowance_actual']!='1'){echo $fullDA_after_policy_1;}
else
 {	
...
?></tbody>
<tfoot><tr><td colspan="5">Includes "expenses on tea/coffee, breakfast, lunch, dinner" and any other incidental expenses.</td><th><?php echo $diff->days . ' Days, ';
echo $diff->h . ' hrs';
?></th>
<th>Total (&#8377;)</th><td><b><?php $total2 = $da_total = array_sum($allAmounts);
echo $total2 . '.00'; ?></b></td></tr></tfoot></table></div></div></div>

<div class="row"><div class="col-md-12 light bordered">
<h4 class="form-section">Conveyance-Car Hire Bills(<span style="color:#005982;">Eligibility- &#8377;<?php
if($con_allo>0){echo $con_allo . '/Day';}
else{echo 'Actual';} ?></span>)</h4>
<div class="row"><?php $total3 = 0; ?>
<table id="conveyance_car" class="table table-hover table-bordered text-center">
<thead><tr class="th_blue"><th>Sr.No.</th><th>Date</th><th>Location From</th>
<th>Location To</th><th>Expense Location</th><th>Book By</th><th>Mode</th>
...
else if ($value['book_by']=="3"){echo "Auto";}
?></td><td><?php
if($value['attachment']!='')
 {
  ?><a class="btn-link" target="_blank" href="<?php echo base_url() . $this->config->item('upload_booking_attch_path') . '/' . $value['attachment']; ?>"><i class="fa fa-eye"></i> View </a><?php
...
</tr></tfoot></table></div></div></div><div class="row">
<div class="col-md-12 light bordered">
<h4 class="form-section">Other Expense Details</h4><div class="row"><?php
$total4=0;
$total5=0;
...
?><tr><td><?php echo $i; ?></td><td id="other_manager_expense_date"><?php
if($request['return_date']!=''){echo date(DATE_FORMAT, strtotime($request['departure_date'])) . " To " . date(DATE_FORMAT, strtotime($request['return_date']));}
else{echo date(DATE_FORMAT, strtotime($request['departure_date']));}
?></td><td>Food Expense</td><td><?php
if(isset($other_manager_expense_location)){echo $other_manager_expense_location;}
?></td><td><?php
...
?></td><td><?php echo $value['remarks'] ?></td><td><?php echo $amount ?></td></tr><?php
...
if(isset($DA_50) and $DA_50>0){if($DA_50==3){$lbl_da_total=0;}}
...
echo $paid_by_comp=array_sum($expensesByCompany);?></th></tr>

<tr><th>Paid By Self</th><th id="lbl_total_claim"><?php echo $paid_by_self = array_sum($OwnExpenses);?></th></tr>

<tr><th>D.A.<?php echo $policy; ?></th><th id="lbl_da_total"><?php echo $lbl_da_total; ?></th></tr>

<tr style="display:none;"><th>Other Expense By Travel Desk</th><th><?php
if(isset($other_manager_expense)){echo $other_manager_expense . '.00';}
?></th></tr>

<tr><th>Travel Advance</th><th class="col-md-3"><?php echo $expense_pending['less_advance']; ?></th></tr>

<tr><th><?php
$balan=$paid_by_self+$lbl_da_total-($expense_pending['less_advance']+$other_manager_expense_food+$other_manager_expense_travel);
if($balan>=0){ ?>Pay to Employee<?php }
else{ ?><span style="color:red;">Employee will pay to company</span><?php }
?></th><th id="your_recived"><?php echo $balan; ?></th></tr></tbody></table></div><?php
if($request['travel_type']=="1"){$travel_mode="Flight";}
elseif($request['travel_type']=="2"){$travel_mode = "Train";}
elseif($request['travel_type']=="3"){$travel_mode = "Car";}
elseif ($request['travel_type']=="4"){$travel_mode = "Bus";}
?><div class="col-md-3 col-xs-12">
<table style="display:none;" class="table table-bordered"><thead>
<tr class="th_blue"><th>Expense Type</th>
<th>Eligible</th><th>Requested</th></tr></thead>
<tbody><tr><td>Travel Mode/Class</td><td><?php
if($eligibility_mode!=''){echo $eligibility_mode."/".$eligibility_class;}
else{if($sel_traverl_class!=''){echo $travel_mode."/".$sel_traverl_class;}
else{echo $travel_mode;}}
...
if($request['hotel_allowance_actual']!=1){echo $hotel_allowance*$day;}
else{echo "Actual";}
?></td><td><?php echo $total1; ?></td></tr>
<tr><td>Conveyance</td><td><?php
if($request['convince_allowance_actual']!=1){echo $convince_allowance*$day;}
else{echo "Actual";}
...
rules: {credit_card_number: {required: true},bank_name: {required: true},},
