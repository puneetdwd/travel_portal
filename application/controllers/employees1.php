...
Class Employees extends Admin_Controller {
    ...
    function add_employee() {
        ...
        if ($this->input->post('sel_emp')) {
            ...
            $employees_data = $this->common->select_data_by_id('employees', 'id', $sel_emp, 'id,gi_email');
            if ($this->session->userdata('emp_list')) {
                $emp_list = $this->session->userdata('emp_list');
            } else {
                $emp_list = array();
...
//        if (!$this->input->is_ajax_request() || $name == '') {
...
//        }
        ...
        $like_array = array('employees.gi_email' => $name);
        $stud_list = $this->employee_model->select_data_by_condition('employees', array(), 'employees.id,employees.gi_email as name', '', '', '10', '', array(), $like_array);
...
//        po($stud_list);
        ...
        foreach ($stud_list as $recSql) {
            ...
            $json_arr["value"] = $recSql['name'];
            $json_arr["label"] = $recSql['name'];
            array_push($display_json, $json_arr);
        }
