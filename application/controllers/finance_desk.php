...
//            $request = $this->finance_desk->get_pending_requests($employee_id);            
    ...
    public function inbox($type = NULL) {
        ...
        $merge_expense_request = $this->finance_desk->get_all_merge_expense_pending_for_manager($employee_id);
        $merge_request_audit_arr = $this->finance_desk->get_travel_merge_requests_unaudit($employee_id);
        $request = array();
        $request_audit = array();
        foreach ($request_arr as $key => $value) {
            $request[$value['id']] = $value;
...
//        po($request_audit);
        ...
        $view_request['merge_request_audit_arr'] = $merge_request_audit_arr;
...
";
    ...
    function merge_expense_pending($exp_id) {
        ...
        $view_request['merge_expense_data'] = $merge_expense_data;
...
//            po($request);
            ...
            if (!empty($other_manager_expense)) {
                ...
                $view_request['other_manager_expense_location'][$request_id] = $other_manager_expense['expense_location'];
...
//        }
        ...
        $this->template->write_view('content', 'finance_desk/merge_expense_pending', $view_request);
        $this->template->render();
    }

    function merge_approve_expense($exp_id = '') {
        $this->load->model("Merge_expense_model", 'merge_expense_model');
        $merge_expense_data = $this->merge_expense_model->get_all_merge_expense_by_id($exp_id);
        ...
        if (empty($merge_expense_data)) {
            ...
            if ($merge_expense_data['request_status'] == '8') {
                        ...
                        if ($this->common->update_data($data_array, 'travel_request', 'id', $request_id)) {
                            ...
                            if ($request['travel_reason_id'] == "Projects") {
                                ...
                                if ($this->common->update_data($data_array, 'projects', 'id', $budget_id)) {
                                    ...
                                    $this->common->insert_data($data_type_array, 'budget_history');
                                }
                            } else {
                                $employee_id = $request['employee_id'];
                                $this->load->model('employee_model');
                ...
                $less_advance_hidd_audit = $this->input->post('less_advance_hidd_finance');
                if ($less_advance_hidd_audit == '') {
                    $less_advance_hidd_audit = 0;
                }
                $your_recived_hidd_audit = $this->input->post('your_recived_hidd_finance');
                if ($your_recived_hidd_audit == '') {
                    $your_recived_hidd_audit = 0;
                }
                $data_array = array(
                ...
                if ($this->common->update_data($data_array, 'merge_expense', 'id', $expense_id)) {
    ...
    function merge_audit_expense($exp_id = '') {
        $this->load->model("Merge_expense_model", 'merge_expense_model');
        $merge_expense_data = $this->merge_expense_model->get_all_merge_expense_by_id($exp_id);
        ...
        if (empty($merge_expense_data)) {
            ...
            if ($merge_expense_data['request_status'] == '6') {
                ...
                if ($this->common->update_data($data_array, 'merge_expense', 'id', $expense_id)) {
                    $this->session->set_flashdata('success', 'Merge Expense Sent to Adut Manager.');
                    redirect(base_url() . 'finance_desk/inbox');
                } else {
                    $this->session->set_flashdata('error', 'Error Occurred. Try Again!');
                    redirect(base_url() . 'finance_desk/inbox');
