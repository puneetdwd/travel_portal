...
class Travel_rejected_reasons extends Admin_Controller {
...
//        $this->is_user_admin();
        ...
        $this->load->model("travel_rejected_reasons_model","travel_reasons_model");
    }
    ...
    public function index() {
        ...
        $this->template->write_view('content', 'travel_rejected_reasons/index_travel_reasons', $view_travel_reasons);
    ...
    public function add_travel_reasons($travel_reasons_id = '') {
        ...
        if ($this->input->post()) {            
            ...
            if (empty($comb_id)) {
                ...
                if ($data) {
                    ...
                    redirect(base_url() . travel_rejected_reasons);
                } else {
        ...
        $this->template->write_view('content', 'travel_rejected_reasons/add_travel_reasons', $view_data);
        $this->template->render();
    }
    ...
    public function status($code, $status, $view = '') {
        ...
        if ($view == 'view') {
            redirect(base_url() . 'travel_rejected_reasons/view/' . $code);
        } else {
            redirect(base_url() . 'travel_rejected_reasons');
        }
    }
